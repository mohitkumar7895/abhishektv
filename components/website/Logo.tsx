    ...
    <Link href="/" className="flex min-w-0 items-center" aria-label={name || "Home"}>
      {photo ? (
        ...
        <span className="flex min-w-0 items-center gap-2.5">
          <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl sm:h-10 sm:w-10 lg:h-11 lg:w-11 ${light ? "bg-copper text-navy" : "bg-navy text-copper"}`}>
            <Tv size={18} strokeWidth={2.2} />
          </span>
          <span className={`max-w-44 text-[0.68rem] font-bold uppercase leading-[1.05] tracking-[0.08em] sm:max-w-52 sm:text-xs ${light ? "text-white" : "text-navy"}`}>
            {name}
          </span>
        </span>
      )}
